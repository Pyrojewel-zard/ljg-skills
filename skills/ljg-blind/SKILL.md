...
写入 `notes/{时间戳}--盲区-{主题}__blind.md`（项目根 `notes/` 目录）。markdown 格式，禁止 org-mode 语法。
...
- 章节：<章节标题>
- 时长：约 N 分钟
- 链接：[在微信读书打开](weread://reading?bId=XXX&chapterUid=YYY)
...
## markdown 严格语法（禁混 org-mode）

- 标题用 `#` / `##` / `###`，不要 `*`
- 加粗 `**字**`，斜体 `*字*`，等宽 `` `code` ``
- 列表用 `-`，不要 `*`（`*` 是加粗）
- 链接 `[text](url)`，不要 `[[url][text]]`
- 分隔线 `---`；引用用 `>`
...
Pulse 每天早上有个自动早信（`陈平安思维拓扑.md`），是陈平安的口气、每天一封、挑一个洞。ljg-blind 是**随手调**的分析版——同一套「读昨天 → 看盲区 → weread 选章」的底子，但更系统、专照盲区、分析写得更足，而且能扫任意一天（传日期参数）。想要一封信，等早信；想主动查某天的盲区、要一篇能存档的分析，用 ljg-blind。
