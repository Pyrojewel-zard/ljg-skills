...
- *互斥格式句不能只替换格式名*——「文件必须是 org-mode，禁止 Markdown」若只把 `org-mode` 换成 `markdown`，会生成「必须是 markdown，禁止 Markdown」的自相矛盾契约。mdize 必须把整句改成「必须是 Markdown，禁止 Org 格式」，残留审计也必须拒绝这种矛盾句
